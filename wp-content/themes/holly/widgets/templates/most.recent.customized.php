...
<div class="bmsbw-container bmsbw-most-recent-customized">
	<?= $before_title ?><span class="bmsbw-title"><?= $title ?></span><?= $after_title ?>
				...
				<?php if (isset($post->thumb) && !empty($post->thumb)): /** if we have a thumb to display */ ?>
					<li class="bmsbw-post-with-thumb">
						<table>
							<tbody>
								<tr>
									<td class="bmsbw-valign-fix-image"><a href="<?= get_permalink($post->ID) ?>" title="Read More" class="bmsbw-image-keyhole bmsbw-40x30-keyhole"><?= ice_get_attachment_image($post->thumb->ID, array(40, 30)) ?></a></td>
	...
	<?php if (isset($see_more) && !empty($see_more)): ?>
		<div class="bmsbw-bottom blue">
			<a class="see-more" href="<?= $see_more ?>">See&nbsp;More&nbsp;&raquo;</a>
			<div class="clear"></div>
		</div>
	<?php endif; ?>
</div>
