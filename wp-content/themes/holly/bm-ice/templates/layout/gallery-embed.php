<div class="bar-gallery midpost-gallery-embed midpost-gallery-<?= $this->args->embed_id ?><?= $this->args->title ? ' with-title' : '' ?>">
    ...
    <h3 class="embed-title" title="Add a title"><?= $title ?></h3>
    <div class="cleaner mceNonEditable"></div>
    <div class="orig-settings mceNonEditable" style="width:0px;height:0px;overflow:hidden;">
        <div class="param"><div class="name">embed_id</div><div class="value"><?= (int)$this->args->embed_id ?></div></div>
        <div class="param"><div class="name">type</div><div class="value">gallery</div></div>
        <div class="param"><div class="name">title</div><div class="value"><?= urlencode($this->args->title) ?></div></div>
    </div>
    <?php /* otherwise just draw the title the way it would normally look */ ?>
    <?php elseif ($this->args->title): ?>
    <a class="bar-title" href="<?= $gal_link ?>"><h3 class="embed-title"><?= urldecode($this->args->title) ?></h3></a>
    <?php endif; ?>
    <div class="bar-list midpost-gallery-embed-inner">
        <div class="bar-list-inner midpost-gallery-embed-row<?= $this->args->preview ? ' mceNonEditable' : '' ?>">
            ...
            <div class="thumb-keyhole gallery-thumbnail-container gallery-thumb-<?= $id ?>-container<?= $this->args->preview ? ' mceNonEditable' : '' ?>">
                <?php $img_classes = 'gallery-thumbnail gallery-thumbnail-'.$id.($this->args->preview ? ' mceNonEditable mceItem' : ''); ?>
                <a href="<?= $link ?>"><?= self::_attachment_image($id, array(94, 94), $img_classes) ?></a>
            </div>
            <?php endforeach; ?>
    ...
    <div class="more-wrapper midpost-gallery-embed-more-link<?= $this->args->preview ? ' mceNonEditable' : '' ?>"><?= $more_photos_link ?></a></div>
    <div class="cleaner"></div>
</div>